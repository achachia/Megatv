...
require dirname(dirname(dirname(__FILE__))) . '/modele/beneficiaires/edit_fiche_intervenant.php';
$control = true;
$objet = array();
$code_intervenant = unhtmlentities($_POST ['code_intervenant']);
...
$id_sex = unhtmlentities($_POST ['id_sex']);
$id_statut = unhtmlentities($_POST ['id_statut']);
...
$code_banque = unhtmlentities($_POST ['code_banque']);
$code_guichet = unhtmlentities($_POST ['code_guichet']);
$n_compte = unhtmlentities($_POST ['n_compte']);
$cle_rib = unhtmlentities($_POST ['cle_rib']);
$id_diplome = unhtmlentities($_POST ['niveau_peda']);
...
if ($code_intervenant == '') {
    ...
    try {
        $select = $cxn->query(" SELECT id_intervenant  FROM intervenants WHERE code_intervenant='" . $code_intervenant . "' ");
        $nb = $select->rowCount();
        if ($nb <= 0) {
            $control = FALSE;
...
if ($control) {
    $req2 = '';
    $infos_beneficiaire = infos_beneficiaire($code_intervenant);
...
//             $sql = " SELECT id_dispo FROM dispo_hebdo_eleve WHERE code_eleve='" . $code_intervenant . "' ";
