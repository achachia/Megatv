...
function generer_code_client($chaine) {
	global $cxn;
	$agent = '';
...
function generer_code_intervenant($chaine) {
	global $cxn;
	$agent = '';
	$chaine_crypte = md5($chaine);
	while ($agent == '') {
		try {
			// On envois la requète
			$select = $cxn->query(" SELECT id_intervenant FROM intervenants  WHERE mot_passe='" . $chaine_crypte . "'  ");
		} catch (Exception $e) {
			echo "Une erreur est survenue lors de la récupération des données";
		}
		$nb = $select->rowCount();
